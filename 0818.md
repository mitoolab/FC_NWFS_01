...
- 1991 ~ 1999: Sir Timothy John "Tim" Berners-Lee가 하이퍼텍스트 기반의 프로젝트를 제안한 이후 정적인 컨텐츠들을 중심으로 한 웹 기술이 발달 (World Wide Web)  
...
> 목록화(리스트업) 할때 많이 사용된다.  
> 리스트업이 되어있는 상태를 요청하여 받는다.  
> 리스트업된 목록을 탐색하는 것  
> 정보를 제공하기 위해선 꾸준히 업데이트를 해야하고, 스크롤 및 액션이 많이 발생하는 단점이 있다.  
...
`Resource` - URI
`Verb` - HTTP method (get, post, put, delet)
`Representations` - 표현

> 표현가능한 상태를 전달하는 것  
> 주소(uri)를 통해 어떤 리소스를 접근하여 불러온다.  
...
> json : 자바스크립트의 객체를 표현할때 사용하는 자료형  
...
---
## Characteristics of REST (특징)
- 범용성(HTTP가 가능하면 OK) : 웹, 앱 모두 대응 가능
...
> 버전관리는 문서화가 중요하다.  
> 정보요청을 뚜렷하게 해야한다.  
> url안에 모든 정보를 집약하여 보낸다.(명사형 작성)  
> 도메인을 별도로 하는 것보다 url안에 포함시는 것이 좋음  
